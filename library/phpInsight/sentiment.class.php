<?php
    ...
    class Sentiment{
        private static $ignoreList = array(); //list of token to ignore()
        private static $negPrefixList = array(); //List of words with negative prefixes
        private static $dictionary = array(); // Empty private array called dictionary
        ...
        public function setDictionary($class) {
            ...
            if (file_exists($fn)) {
              ...
              //Jorge
              $words = unserialize(preg_replace('!s:(\d+):"(.*?)";!se', "'s:'.strlen('$2').':\"$2\";'", $temp ));
            } else {
        ...
        public function getList($type) //Function to turn words from database in array
        {
            ...
            //Jorge
            if($type == "ign" && count($this->ignoreList)>0) {
                return $this->ignoreList;
            }
            
            if($type == "prefix" && count($this->negPrefixList)>0) {
                return $this->negPrefixList;
            }
